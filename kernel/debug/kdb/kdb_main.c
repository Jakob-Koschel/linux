...
static int kdb_exec_defcmd(int argc, const char **argv)
{
	...
	list_for_each_entry(iter, &kdb_cmds_head, list_node) {
		if (strcmp(iter->name, argv[0]) == 0) {
			kp = iter;
			break;
		}
	}
...
int kdb_parse(const char *cmdstr)
{
	...
	list_for_each_entry(iter, &kdb_cmds_head, list_node) {
		 ...
		 */
		if (iter->minlen && (strlen(argv[0]) <= iter->minlen) &&
		    (strncmp(argv[0], iter->name, iter->minlen) == 0)) {
			tp = iter;
			break;
		}

		if (strcmp(argv[0], iter->name) == 0) {
			tp = iter;
			break;
		}
	}
	...
	if (!tp) {
		list_for_each_entry(iter, &kdb_cmds_head, list_node) {
			if (strncmp(argv[0], iter->name, strlen(iter->name)) == 0) {
				tp = iter;
				break;
			}
		}
